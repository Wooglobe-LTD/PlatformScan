...
class Profile_Model extends APP_Model {
    ...
    public function getAdminBysimpleLink($link)
    {
        $query = 'SELECT * FROM admin where simple_link= "'.$link.'" AND deleted = 0';
        $result = $this->db->query($query);
        return $result;
    ...
    public function paid($id)
    {
        ...
        $query = "SELECT * FROM currency WHERE code = 'USD' LIMIT 1";
        $result = $this->db->query($query)->result_array();
        $default_currency = '';
        if(count($result) > 0){
    ...
    {
        $query = 'SELECT e.*,v.title,c.symbol
                   FROM videos v 
                   INNER JOIN earnings e 
                   ON v.id = e.video_id 
                   AND e.status = 1
    ...
    public function getRevenueGroupNew($id,$slug = NULL)
    {
        $query = 'SELECT e.*,v.title,c.symbol,v.slug url
                   FROM videos v 
