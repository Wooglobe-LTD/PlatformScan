...
class Home_Model extends APP_Model {
    ...
    {
        $query = "SELECT v.youtube_id,v.title as vtitle,v.category_id,v.tags,v.user_id,v.description,ev.portal_url AS url,ev.portal_thumb AS thumbnail,v.created_at,v.slug,v.embed,v.id,u.full_name,u.picture,c.title as ctitle
                  FROM `videos` v
    ...
    public function list_videos($user_id = 0)
    {
        ...
        $query = "SELECT * FROM videos WHERE user_id = ".$user_id." AND (youtube_id IS NOT NULL AND youtube_id != '') ORDER BY  created_at DESC LIMIT 1,3";
    ...
    public function view_count_video($video_id){
        ...
        $result = $this->db->query('
            ...
            OR ip_address = "'.$ip.'"
        ');
        $res = $this->db->query('SELECT COUNT(id) as msg_found FROM videos_views WHERE ip_address="'.$ip.'"')->result();
        $row_ip = $res[0];
