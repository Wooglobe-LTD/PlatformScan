...
class VideoDislikeViewController extends API_Controller
{
    ...
    public function dislikefetch()
    {
        $formData = json_decode($this->input->raw_input_stream, true);
        $user_id=$formData['user_id'];
        $video_id=$formData['video_id'];
        $res = VideoExpression::where(['user_id'=>$user_id, 'video_id'=>$video_id,'is_dislike'=>1])->get();
        $row = $res->count();
        if ($res) {
            $data = "False";
            if($row > 0){
