...
$(function() {
    ...
    $(document).on('click', '#mrss-edit-btn', function(e){
        e.preventDefault();
        var id = $(this).data('id');
        $.ajax({
            type: "POST",
            url: base_url + "mrss_feed_edit",
            data: {mrss_id:id},
            success: function (res) {
                res = JSON.parse(res);
                if (res.code == 200) {
                    $('#story_title').val(res.data.story_title);
                    $('#story_description').val(res.data.story_description);
                    $('#story_tags').val(res.data.story_tags);
                    $('#exclusive-partners-list-story option[value='+res.data.partner_id+']').attr('selected', 'selected');
                    $('#story_brand_id option[value='+res.data.brand_id+']').attr('selected', 'selected');
                    $('#mrss_edit').val('1');
                    $('#mrss_id').val(id);
                    var modal = UIkit.modal("#mrss-upload-modal");
                    modal.show();
                } else {
                    $('#err-msg').attr('data-message', 'Something is going wrong!');
                    $('#err-msg').click();
    ...
    $(document).on('click', '#mrss-delete-btn', function(e){
        e.preventDefault();
        var id = $(this).data('id');
        $.ajax({
            type: "POST",
            url: base_url + "mrss_feed_delete",
            data: {mrss_id:id},
            success: function (res) {
                res = JSON.parse(res);
                if (res.code == 200) {
                    $('#suc-msg').attr('data-message', 'Deleted Successfully!');
                    $('#suc-msg').click();
                    $('#mrss-task-'+id).remove();
                } else {
                    $('#err-msg').attr('data-message', 'Something is going wrong!');
    ...
    $(document).on('click', '#mrss-upload-btn', function(e){
        e.preventDefault();
        var modal = UIkit.modal("#mrss-upload-modal");
        modal.show();
    });
    $(document).on('click','#mrss-upload-save',function(){
        $('#story_content_form').submit();
    });
    $(document).on('submit','#story_content_form',function(e){
        e.preventDefault();

        $("#mrss-progress").css('display', 'block');
        var wgid = $('#wgid').val();
        var title = "<b>Title: </b>";
        var description = "<b>Description: </b>";
        var tags = '<b>Tags: </b>'
        var partner = "<b>Partner: </b>";
        var brand = "<b>Brand: </b>";
        var video = "<b>Video URL: </b>";
        var thumb = "<b>Thumb URL: </b>";
        var thumb_url = '';
        var mrss_id = $('#mrss_id').val();

        title += $('#story_title').val();
        description += $('#story_description').val();
        partner += $('#exclusive-partners-list-story').find('option:selected').text();
        if($('#story_brand_id').find('option:selected').val() != null) {
            brand += $('#story_brand_id').find('option:selected').text();
        }
        var html = '';
        // custom_preloader_hide();
        $.ajax({
            type: "POST",
            url: base_url + "save_story_content",
            ...
            xhr: function() {
                ...
                xhr.upload.addEventListener("progress", function(evt) {
                    if (evt.lengthComputable) {
                        ...
                        $('#mrss-card-progress').css('width', percentComplete + '%');
                    }
                }, false);
                return xhr;
            },
            success: function (data) {
                ...
                if (data.code == 204) {
                    ...
                    if (data.code == 200) {
                        ...
                        for(let d in rows){
                            video += rows[d].story_video_path;
                            thumb += rows[d].story_thumb_path;
                            thumb_url = rows[d].story_thumb_path;
                            if (rows[d].story_thumb_path.length < 1)
                            {
                                thumb_url = $("#lead_thumbnail_url").attr('href');
                            }
                            tags += rows[d].story_tags;
                            if (data.id != null) {
                                html = 
                                '<div id="mrss-task-'+data.id+'" class="mrss-task">' +
                                    '<div class="mrss_thumb">' +
                                        '<img src="'+thumb_url+'" alt="Thumbnail"/>' +
                                    '</div>' +
                                    '<p class="mrss_details">'+title+'</p>' +
                                    '<div class="mrss_desc">'+description+'</div>' +
                                    '<p class="mrss_details">'+tags+'</p>' +
                                    '<p class="mrss_details">'+partner+'</p>' +
                                    '<p class="mrss_details">'+brand+'</p>' +
                                    '<p class="mrss_details">'+video+'</p>' +
                                    '<p class="mrss_details">'+thumb+'</p>' +
                                    '<div class="mrss_card_btn">' +
                                        '<button id="mrss-edit-btn" data-id="'+data.id+'"><i class="material-icons" style="color:#ffffff">edit</i></button>' +
                                        '<button id="mrss-delete-btn" data-id="'+data.id+'"><i class="material-icons" style="color:#ffffff;">delete</i></button>' +
                                    '</div>' +
                                '</div>';
                                $(html).insertBefore('#add_mrss')
                            }
                            else if (mrss_id != '') {
                                html = 
                                    '<div class="mrss_thumb">' +
                                    ...
                                    '<p class="mrss_details">'+thumb+'</p>' +
                                    '<div class="mrss_card_btn">' +
                                        '<button id="mrss-edit-btn" data-id="'+mrss_id+'"><i class="material-icons" style="color:#ffffff">edit</i></button>' +
                                        '<button id="mrss-delete-btn" data-id="'+mrss_id+'"><i class="material-icons" style="color:#ffffff;">delete</i></button>' +
                                    '</div>';
                                $('#mrss-task-'+mrss_id).html(html);
                            }
                        }
                        $('#suc-msg').attr('data-message', data.message);
                        $('#suc-msg').click();
                    ...
                    $('#mrss_edit').val('0');
                    $('mrss_id').val('');
                    $('#mrss-progress').css('display', 'none');
                    var modal = UIkit.modal("#mrss-upload-modal");
                    modal.hide();
                    $('#story_content_form').trigger("reset");
                }
            },
            error: function () {
                $('#mrss-progress').css('display', 'none');
    ...
    $('#scout-resolve-issue').on('click',function (){
        var id = $(this).data('id');
        var conf = confirm('Are you sure to mark as resolved?')
        if(conf){
            $.ajax({
                type: "POST",
                url: base_url + "scout_report_bug_resolved",
                data: {id:id},
                success: function (data) {
                    //data = JSON.parse(data);
                    if (data.code == 204) {
    ...
    $(document).on('submit', '#res-att-form', function(e) {
        e.preventDefault();
        var id = $("#upload-res").data('id');
        var res_directory = $("#upload-res").data('dir');
        var files = [];
        $('#res-att-table tr').each(function() {
            files.push($(this).find("td").eq(0).html());    
        });
        $.ajax({
            type: "POST",
            url: base_url + "res-att-upload/" + id,
            data: new FormData(this),
            contentType: false,
            cache: false,
            processData:false,
            success: function(data)
            {
                for (var i = 0; i < $('#res-att').get(0).files.length; ++i) {
                    var num = 1;
                    var filename = $('#res-att').get(0).files[i].name;
                    var ext = filename.split('.').pop().toLowerCase();
                    var oldname = filename.split('.')[0];
                    while (files.includes(filename.toLowerCase()))
                    {
                        var rep = "-"+num+".";
                        filename = oldname+rep+ext;
                        num++;
                    }
                    $("#res-att-table").append("<tr><td>"+filename.toLowerCase()+"</td><td><a href='../../"+res_directory+"/"+filename+"' download><span class='material-icons' style='color:#38f;'>file_download</span></a></td><td><a class='remove-att' data-path="+res_directory+" data-file="+filename+"><span class='material-icons' style='color:#38f;'>delete</span></a></td></tr>");
                }; 
                var modal = UIkit.modal("#res-att-modal");
                modal.hide();
                $('#suc-msg').attr('data-message','Uploaded Successfully!');
                $('#suc-msg').click();
            },
            error: function(e) 
            {
                $('#err-msg').attr('data-message', html(e));
                $('#err-msg').click();
            }          
        });
    });

    $(document).on('submit', '#mgr-att-form', function(e) {
        e.preventDefault();
        var id = $("#upload-mgr").data('id');
        var mgr_directory = $("#upload-mgr").data('dir');
        var files = [];
        $('#mgr-att-table tr').each(function() {
            files.push($(this).find("td").eq(0).html());    
        });
        $.ajax({
            type: "POST",
            url: base_url + "mgr-att-upload/" + id,
            data: new FormData(this),
            contentType: false,
            cache: false,
            processData:false,
            success: function(data)
            {
                for (var i = 0; i < $('#mgr-att').get(0).files.length; ++i) {
                    var num = 1;
                    var filename = $('#mgr-att').get(0).files[i].name;
                    var ext = filename.split('.').pop().toLowerCase();
                    var oldname = filename.replace("."+ext, "");
                    while (files.includes(filename.toLowerCase()))
                    {
                        var rep = "-"+num+".";
                        filename = oldname+rep+ext;
                    ...
                    $("#mgr-att-table").append("<tr><td>"+filename.toLowerCase()+"</td><td><a href='../../"+mgr_directory+"/"+filename+"' download><span class='material-icons' style='color:#38f;'>file_download</span></a></td><td><a class='remove-att' data-path="+mgr_directory+" data-file="+filename+"><span class='material-icons' style='color:#38f;'>delete</span></a></td></tr>");
                }; 
                var modal = UIkit.modal("#mgr-att-modal");
                modal.hide();
                $('#suc-msg').attr('data-message','Uploaded Successfully!');
                $('#suc-msg').click();
            },
    ...
    $(document).on('change', '#res-att', function(e) {
        if ($('#res-att').get(0).files.length > 0) {
            ...
            if($(this).val() != '') {
                styleElem.innerHTML = "#res-att:before {content: '"+files+"';}";
            }
            else {
                var styleElem = document.head.appendChild(document.createElement("style"));
                styleElem.innerHTML = "#res-att:before {content: 'File Not Attached';}";
            }
        }
    });

    $(document).on('change', '#mgr-att', function(e) {
        if ($('#mgr-att').get(0).files.length > 0) {
            var styleElem = document.head.appendChild(document.createElement("style"));
            var files = "";
            for (var i = 0; i < $('#mgr-att').get(0).files.length; ++i) {
                if (i != 0) { files = files = files+", "; }
                var filename = $('#mgr-att').get(0).files[i].name;
                files = files+filename;
            };
            if($(this).val() != '') {
                styleElem.innerHTML = "#mgr-att:before {content: '"+files+"';}";
            }
            else {
                var styleElem = document.head.appendChild(document.createElement("style"));
                styleElem.innerHTML = "#mgr-att:before {content: 'File Not Attached';}";
            }
        }
    });

    $(document).on('click', '.remove-att', function(e) {
        var path = $(this).data('path');
        var filename = $(this).data('file').toLowerCase();
        var file = path + '/' + filename;
        var row = $(this).parent().parent();
        $.ajax({
            type: "POST",
            url: base_url + "remove-att",
            data: {file:file},
            success: function(response)
            {
                row.remove();
                $('#suc-msg').attr('data-message','File Deleted!');
                $('#suc-msg').click();
            },
            error: function(e) 
            {
    ...
    if(access.list) {
		...
		$('.toogle-order').on('click, change', function () {
            ...
            var col = $(this).data('colvalue');
			$(this).parent().parent().find('a.refresh-sort').data('sort', order);
			$(this).parent().parent().find('a.refresh-sort').data('col', col);
		});
...
///////////////////////////////////Update Video Title///////////////////////////////////////
        ...
        async function update_title_2() {
            var text_2 = $("#edit-title-area-2").data('val');
            if (text_2){
                return "";
            }
            var text = $("#edit-des-area").data('val');
            // var prompt = `Rewrite the sentence (${text})`;
            var url=(window.location.href).split('/');
            var id = url[url.length-1];
            $.ajax({
                type:"GET",
                url: base_url + 'get_configurations',
                success: function (data) {
                    data = JSON.parse(data);
                    var prompt = data["title_2_prompt"];
                    prompt = prompt.replace('${text}', text);
                    console.log(prompt);
                    res = getGPTResponse(prompt).then(res => {
                        $.ajax({
                            type:"POST",
                            url: base_url + 'update_title_2',
                            data: {id:id,content:res},
                            success: function (data) {
                                data = JSON.parse(data);
                                if (data.code == 200) {
                                    $("#edit-title-area-2").html(res);
        ...
        $('#cancel-title').click(function(){
            var cancel=$('textarea#title-text').val();
        ...
        $('#save-title').click(async function(){
            $('#save-title').hide();
            $('textarea#title-text').each(async function(){
        ...
        ///////////////////////////////////Update Video Title 2///////////////////////////////////
        $('#edit-title-2').click(function(){
            $('#edit-title-2').hide();
            $('span.title-area-2').each(function(){
                var content = $(this).html();
                $(this).html('<textarea id="title-text-2">' + content + '</textarea>');
            });
            $('#cancel-title-2').show();
            $('#save-title-2').show();
        });
        $('#cancel-title-2').click(function(){
            var cancel=$('textarea#title-text-2').val();
            var title_area = $('#edit-title-area-2');
            var date_val = title_area.data('val');
            title_area.html(date_val);
            $('#edit-title-2').show();
            $('#cancel-title-2').hide();
        ...
        $('#save-title-2').click(async function(){
            $('#save-title-2').hide();
            $('textarea#title-text-2').each(async function(){
                var title = $(this).val();//.replace(/\n/g,"<br>");
                var updatetitle= $(this).html(title);
                $(this).contents().unwrap();
                var url=(window.location.href).split('/');
        ...
        ///////////////////////////////////Update SEO Target Words///////////////////////////////////////
        $('#edit-seo-targets').click(function () {
            $('#edit-seo-targets').hide();
            $('span.seo_targets_area').each(function () {
                var content = $(this).html();
                $(this).html('<textarea id="seo-targets-text">' + content + '</textarea>');
            });
            $('#cancel-seo-targets').show();
            $('#save-seo-targets').show();
        });
        $('#cancel-seo-targets').click(function () {
            var cancel = $('textarea#seo-targets-text').val(); console.log(cancel);
            var title_area = $('#seo-targets-area');
            var date_val = title_area.data('val');

            title_area.html(date_val);
            $('#edit-seo-targets').show();
            $('#cancel-seo-targets').hide();
            $('#save-seo-targets').hide();
        });
        $('#save-seo-targets').click(function () {
            $('#save-seo-targets').hide();
            $('textarea#seo-targets-text').each(function () {
                var seo_targets = $(this).val();
                $(this).contents().unwrap();
                var url = (window.location.href).split('/');
                var id = url[url.length - 1];
                $.ajax({
                    type: "POST",
                    url: base_url + 'update_seo_targets',
                    data: { id: id, content: seo_targets },
                    success: function (data) {
                        data = JSON.parse(data);
                        if (data.code == 200) {
                            $('#suc-msg').attr('data-message', data.message);
...
///////////////////////////////////Update Video Rating Comments///////////////////////////////////////
        ...
        $('#edit-q0').click(function(){
            $('#edit-q0').hide();
            $('span.q0-area').each(function(){
                var content = $(this).html();
                $(this).html('<textarea id="q0-text">' + content + '</textarea>');
            });
            $('#cancel-q0').show();
            $('#save-q0').show();
        });
        $('#cancel-q0').click(function(){
            var cancel=$('input#q0-text').val();console.log(cancel);
            var q1_area = $('#edit-q0-area');
        ...
        $('#save-q0').click(function(){
            $('#save-q0').hide();
            $('textarea#q0-text').each(function(){
                var q0_comments = $(this).val();
                var trimStr = $.trim(q0_comments);
                $(this).contents().unwrap();
                var url=(window.location.href).split('/');
                var id = url[url.length-1];
                if(trimStr){
                    $.ajax({
                        type:"POST",
                        url: base_url + 'update_q0',
                        data: {id:id,content:q0_comments},
                        success: function (data) {
                            data = JSON.parse(data);
                            if (data.code == 200) {
                                $('#suc-msg').attr('data-message', data.message);
...
*/
    ...
    $(document).on('change', '#cn-check', function(e){
        e.preventDefault();
        if($("#cn-check").is(":checked")) {
            $("#cn-check").prop("checked", false);
        }
        else {
            $("#cn-check").prop("checked", true);
        }
        var modal = UIkit.modal("#cn-modal");
        modal.show();
    });
    $(document).on('change','#ve-check', function(e){
        e.preventDefault();
        if($("#ve-check").is(":checked")) {
            $("#ve-check").prop("checked", false);
        }
        else {
            $("#ve-check").prop("checked", true);
        }
        var modal = UIkit.modal("#ve-modal");
        modal.show();
    });
    $(document).on('change', '#trending-check', function(e){
        e.preventDefault();
        if($("#trending-check").is(":checked")) {
            $("#trending-check").prop("checked", false);
        }
        else {
            $("#trending-check").prop("checked", true);
        }
        var modal = UIkit.modal("#trending-modal");
        modal.show();
    });
    $(document).on('change', '#ai-based-check', function(e){
        e.preventDefault();
        if($("#ai-based-check").is(":checked")) {
            $("#ai-based-check").prop("checked", false);
        }
        else {
            $("#ai-based-check").prop("checked", true);
        }
        var modal = UIkit.modal("#ai-based-modal");
        modal.show();
    });

    $(document).on('click', '#update-cn', function(e){
        var id  = $(this).data('id');
        var check = 0;
        var modal = UIkit.modal("#cn-modal")
        if(!($("#cn-check").is(":checked"))) {
        ...
        $.ajax({
            method: "POST",
            url: base_url + 'cn-update',
            data : {lead_id:id, checked:check},
            success: function (data) {
                if($("#cn-check").is(":checked")) {
                    $("#cn-check").prop("checked", false);
                }
                ...
                modal.hide();
                $('#suc-msg').attr('data-message', 'Record Updated Successfully!');
                $('#suc-msg').click();
            }
        });
    });
    $(document).on('click', '#update-ve', function(e){
        var id  = $(this).data('id');
        var check = 0;
        var modal = UIkit.modal("#ve-modal");
        if(!($("#ve-check").is(":checked"))) {
    ...
    $(document).on('click', '#update-trending', function(e){
        var id  = $(this).data('id');
        var check = 0;
        var modal = UIkit.modal("#trending-modal");
        if(!($("#trending-check").is(":checked"))) {
    ...
    $(document).on('click', '#update-ai-based', function(e){
        var id  = $(this).data('id');
        var check = 0;
        var modal = UIkit.modal("#ai-based-modal");
        if(!($("#ai-based-check").is(":checked"))) {
    ...
    $(document).on('click', '#add-res-att', function(e){
        e.preventDefault();
        var modal = UIkit.modal("#res-att-modal");
        modal.show();
    });

    $(document).on('click', '#add-mgr-att', function(e){
        e.preventDefault();
        var modal = UIkit.modal("#mgr-att-modal");
        modal.show();
    });

    $(document).on('click', '#save-res-comm', function(e){
        var id  = $(this).data('id');
        var comm_value = $("#researcher-comm").val();
        $.ajax({
            method: "POST",
            url: base_url + 'save-res-comm',
            data : {id:id, comment:comm_value},
            success: function (data) {
            }
        });
    });

    $(document).on('click', '#save-mgr-comm', function(e){
        var id  = $(this).data('id');

        var comm_value = $("#manager-comm").val();
        $.ajax({
            method: "POST",
            url: base_url + 'save-mgr-comm',
            data : {id:id, comment:comm_value},
            success: function (data) {
            }
        });
    ...
    $('#priority').on('change', function(){
        ...
        $.ajax({
            method: "POST",
            url: base_url + 'update-priority',
            data : {id:id, selected:value},
            success: function (data) {
    ...
    $(document).on('click', '#btn_update_auth_token', function(e){
        // var elem = $("#youtube_channel").sele;
        channel = $('#youtube_channel').selectize();
        if (channel.length > 0) {
            channel = channel[0].selectize;
            var chan_id = channel.getValue()
            
            if (chan_id == 'UCbbtHuBeqqlRB9yNr_Hpc7w'){
                window.open('../cb_mgmt_channel', '_blank');
            }
            else if (chan_id == 'UCtfC0pUgtWzJuaecnm6l_ng'){
                window.open('../cb_gmail', '_blank');
            }
        }
    });
    $(document).on('click','#publish-to-feeds',function(e){        
        var confirmed = confirm("This video will be published directly to all MRSS feeds where it is enqueued.");
        if(confirmed){
            e.preventDefault();
            var video_id  = $(this).data('id');
            $.ajax({
                url: base_url + 'publish-to-feeds',
                data : {video_id:video_id},
                type: "POST",
                success: function (data) {
                    data = JSON.parse(data);
    ...
    if(access.can_delete_lead) {
        ...
        $(document).on("click", "div[class$='_email_preview']", function () {
            var preview_id = $(this).data('id');
            $(".email_preview").css('display', 'none');
            $("#"+preview_id).css('display', 'block');
        });
        $(document).on('change', '.email_preview_check', function(e){
            e.preventDefault();
            $.ajax({
                type 	: 'POST',
                url  	: base_url+'delete_deal_emails',
                data    : $('#form_validation6').serialize(),
                success : function(data){
                    data = JSON.parse(data);
                    if(data.code == 201){
                        ...
                        $("#email_preview_btns").empty();
                        $("#cancellation_email_preview").empty();
                        for (i in emails) {
                            let class_name = i == "owner"? "owner": "partner";
                            var html = '<div class="'+class_name+'_email_preview" data-id="'+i+'"><button type="button" class="md-btn-info">'+emails[i].name+'</button></div>';
                            $("#email_preview_btns").append(html);
                            
                            html = 
                                '<div id="'+i+'" class="email_preview" style="display:none;">'+
                                    '<div class="uk-form-label email_preview_label"><u><b>From:</b></u></div>'+
                                    '<div>'+emails[i].from+'</div>'+
                                    '<div class="uk-form-label email_preview_label"><u><b>To:</b></u></div>'+
                                    '<div>'+emails[i].to+'</div>'+
                                    '<div class="uk-form-label email_preview_label"><u><b>Subject:</b></u></div>'+
                                    '<div>'+emails[i].subject+'</div>'+
                                    '<div class="uk-form-label email_preview_label"><u><b>Message:</b></u></div>'+
                                    '<div>'+emails[i].message+'</div>'+
                                '</div>';
                            $("#cancellation_email_preview").append(html);
                        }
                        $("div[class$='_email_preview']:first-child").click();
                    }else{
                        $('#err-msg').attr('data-message','Something is going wrong!');
                        $('#err-msg').click();
                    }
    ...
    $(document).on('click', '.upload_dropbox', function (e) {
        var id = $("#dropbox_unique_key").val();
        $.ajax({
            type 	: 'POST',
            ...
            success : function(data){
                ...
                if(data.code == 404){
                    ...
                    if (paths.length > 0) {
                        ...
                        $('#dropbox-search-status').html('* Video does not exist in any dropbox folder');
                    }
        ...
        $('#dropbox-error').html('');
        $('input[name="upload_dropbox_choice"]').prop('checked', false);
        var modal = UIkit.modal("#dropbox-model");
        modal.show();
    });
    $('input[type=radio][name="upload_dropbox_choice"]').change(function(){
        if(this.value == "upload_only_sheet"){
            addRow();
        }else{
            $("#dropbox_sheet_table").empty();
        }
    });
    $(document).on('click', '.dropbox_sheet_btn_add', function(){
        addRow();
    });
    function addRow()    
    {    
        var wgid = $("#dropbox_unique_key").val()
        var table = document.getElementById("dropbox_sheet_table"); //get the table      
        ...
        if (rowcount == 0){
            ...
            $("#dropbox_sheet_table").append(tblHead);    

        }
        var sum_duration = $("#sum_duration").html();

        var tblRow = '<tr id="dropbox_sheet_row' + rowcount + '">'
        + '<td><input type="text" class="md-input" placeholder="Youtube URL" id="sheet_youtube_url' + rowcount + '" /> </td>' 
        + '<td><input type="text" class="md-input" placeholder="00:00" name="sheet_timestamp_from" id="sheet_timestamp_from' + rowcount + '" value="00:00" /></td>'
        + '<td><input type="text" class="md-input" placeholder="00:00" name="sheet_timestamp_to" id="sheet_timestamp_to' + rowcount + '" value="' + sum_duration.replace(".", ":") + '" /></td>' 
        + '<td><input type="text" class="md-input" placeholder="WGID" value="'+ wgid +'"id="sheet_wgid' + rowcount + '" /></td>' 
        + '<td><input type="text" class="md-input" placeholder="Predefined Text" value="WooGlobe M DRM" id="sheet_predefined' + rowcount + '" /></td>' 
        + '<td><input type="text" class="md-input" placeholder="Dropbox URL" id="sheet_dropbox_url' + rowcount + '" /></td>' 
        + '<td><input type="button" class="md-btn md-btn-danger" id="btnDelete' + rowcount + '" onclick="$(\'#dropbox_sheet_row'+ rowcount+'\').remove()" value="Delete" /></td>' 
        + '<td><input type="button" class="md-btn md-btn-info dropbox_sheet_btn_add" id="btnAdd' + rowcount + '" value="Add New" /></td>' 
        + '</tr>';

        $("#dropbox_sheet_table").append(tblRow);    
    } 

    $(document).on('click', '#dropbox_model_submit', function(e){
        var id = $("#dropbox_unique_key").val();
        var title = $("#dropbox_title").val();
        var selected_option = $('input[name="upload_dropbox_choice"]:checked').val();

        var rows = [];
        if(selected_option == "upload_only_sheet"){
            let table = document.querySelector('#dropbox_sheet_table');
            let headers = [...table.rows[0].cells].map(th => th.innerText);
            for(let row of [...table.rows].slice(1, table.rows.length)) {
                let product = Object.fromEntries(new Map([...row.cells].map((cell, i) => [headers.at(i), cell.children[0].value])));
                rows.push(product);
            }
            console.log(rows);
        }   
        ...
        if(selected_option === null || selected_option === undefined){
            $('#dropbox-error').html('');
            $('#dropbox-error').html('Please select appropriate option before submitting!');
        }
        else{
            $('#dropbox-error').html('');
            UIkit.modal("#dropbox-model").hide();
            $.ajax({
                type: "POST",
                url: base_url + "upload_to_dropbox",
                data: {'id': id, 'title': title, 'choice': selected_option, 'rows': rows},
                success: function (data) {
                    console.log(data);
                    data = JSON.parse(data);
                    if (data.code == 200) {
                        ...
                        console.log();
                        msg = Object.keys(data.message).join(', ') + " Added to uploading queue";
                        ...
                        $('#err-msg').attr('data-message', 'Token refreshed. Please re-upload the video.');
                        $('#err-msg').click();
                    }
                    else if (data.code == 201) {
                        $('#err-msg').attr('data-message', data.error);
                        $('#err-msg').click();
                    }
                },
    ...
    $('.scrum_column').scroll(function(event){
        ...
        var order_by = "";
        var sort = "";
        $(this).parent().find('.refresh-sort').each(function(index, element){
            if ($(element).data('id') == scrum_id)
            {
                order_by = $(element).data('col');
            ...
            if ((diff == $(this).outerHeight()) || (diff + 1 == $(this).outerHeight()) || (diff - 1 == $(this).outerHeight())) { // if scrolled to bottom
                if (stage_list[obj_index].max_offset_reached != 1) {
                    ...
                    $.ajax({
                        ...
                        data: {'deal_stage':scrum_id, 'limit':stage_list[obj_index].limit, 'offset':stage_list[obj_index].offset, 'order_by':order_by, 'sort':sort},
                        success: function (response) {
                            response = JSON.parse(response);
    ...
    $(".raw-list li").each(function() {
        var status = $(this).data('dropbox-status');
        var id = $(this).data('lead-id');
        fetch_dropbox_status(status, id);
    });
    function fetch_dropbox_status (status, id) {
        if (status != 'success' && status != 'failed' && status != null) {
            $.ajax({
                type 	: 'POST',
                url  	: base_url+'get_dropbox_job_status',
                ...
                success : function(data) {
                    ...
                    if(data.code == 301) {
                        ...
                        $('#err-msg').attr('data-message', 'Token refreshed.');
                        $('#err-msg').click();
                    }
                    else if(data.code == 200) {
                        var new_status = data.data;
                        var _color = "black";
                        var _status = "";
                        var info = $("#dropbox_status_info").html().split(" ==&gt; ");
                        if (new_status == null) {
                            _color = "blue";
                            _status = "Not Uploaded Yet!";
                        }
                        ...
                        if (new_status == 'failed' || new_status == 'complete' || new_status == 'in_progress') {
                            $("#dropbox_status_info").css({ 'color': _color });
                            $("#dropbox_status_info").html(_status + " ==> " + info[1].trim());
                        }
                    }
                    else{
                        $('#err-msg').attr('data-message','Something is going wrong in fetching dropbox status!');
                        $('#err-msg').click();
                    }
                }
            });
    ...
    $('#gnrt-appearance-rls-btn').click(function(){
        $('#gnrt-appearance-rls-btn').hide();
        $('#gnrt-appearance-rls-div').show();
    });
    $('#cancel-apr-rls').click(function(){
        var rls_desc_area = $('#appearance-rls-desc');
        var date_val = rls_desc_area.data('val');
        rls_desc_area.html(date_val);
        $('#gnrt-appearance-rls-div').hide();
        $('#gnrt-appearance-rls-btn').show();
    });
    $('#gnrt-apr-rls').click(function(e) {
        e.preventDefault();
        var release_description = $('#appearance-rls-desc').val();
        var lead_id = $(this).data('id');
        $.ajax({
            type: "POST",
            url: base_url + 'generate_appearance_rls',
            data: {release_description:release_description,lead_id:lead_id},
            success: function (data) {
                data = JSON.parse(data);
                if (data.code == 200) {
                    $('#suc-msg').attr('data-message', data.message);
...
// brand_elem = $('#story_brand_id').selectize();
// if(brand_elem.length > 0){
//     brand_elem = brand_elem[0].selectize;
// }

// function partner_brand(){
//     var id = $('#exclusive-partners-list-story').val();
//     var dataarray = [];
//     var select_options = '';
//     $('#story_feed_id').html();
...
//                     for (i = 0; i < dataarray.length; i++) {
//                         if (option_ids.indexOf(dataarray[i].id) == -1) {
//                             select_options += '<option value="'+dataarray[i].brand_id+'">'+dataarray[i].brand_name+'</option>';
//                         }
//                     }
//                     $('#story_brand_id').html(select_options);
//                 }
//             }
//         });
//     }
