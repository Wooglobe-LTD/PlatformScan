...
$(function() {
    ...
    appearance = $('#appearance_e').selectize();

    if(appearance.length > 0){
        appearance = appearance[0].selectize;
    }

    appearanceAdd = $('#appearance').selectize();

    if(appearanceAdd.length > 0){
        appearanceAdd = appearanceAdd[0].selectize;
    }
			...
			success : function(data){
				...
				if(data.code == 204){
					...
					$.each(data.data,function(i,v){
                        ...
                        if($('#'+i+'_e').hasAttr('select-w')){
                            watermark.setValue(v);
                        }
                        if($('#'+i+'_e').hasAttr('select-a')){
                            appearance.setValue(v);
                        }
    ...
    $(document).on('click','.add_partner_email',function() {
        var id = $(this).data('id');
        $('.error').html('');
        $('#add_email_form').parsley().reset();
        $('#add_email_form')[0].reset();
		$('#partner_id').val(id);
		$("#partner_email_list").empty();
		$.ajax({
            type: 'POST',
            url: base_url+'get_partner_emails',
			data: {id:id},
            success: function(data) {
				data = JSON.parse(data);
                if (data.code == 200 && data.emails) {
                    for (var email of data.emails) {
                        $("#partner_email_list").append('<li>' + email + '</li>');
                    };
                }
            }
        });
		var modal = UIkit.modal("#add_email_modal");
		modal.show();
    });
	$("#add_email_btn"). on("click", function() {
		$("#add_email_form").submit();
	});
	$('#add_email_form').on('submit',function(e) {
		...
		$.ajax({
			type 	: "POST",
			url  	: base_url+"add_partner_email",
			data    : $('#add_email_form').serialize(),
			success : function(data){
				data = JSON.parse(data);
				if(data.code == 200) {
					$('#suc-msg').attr('data-message',data.message);
    ...
    dt_tableExport: function() {
        ...
        if($dt_tableExport.length) {
            table_export = $dt_tableExport.DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": { url : base_url+'get_partners',contentType: "json"},
