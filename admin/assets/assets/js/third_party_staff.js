...
$(function() {
    ...
    $('#form_validation2').on('submit',function(e){
        ...
        e.preventDefault();
        var formData = new FormData( $('#form_validation2')[0]);
        console.log(formData);
		
		$.ajax({
			type 	: "POST",
			url  	: base_url+"add_third_party_member",
            processData: false,
            contentType: false,
            data    : formData,
	...
	$(document).on('click','.edit-member',function(){
		...
		$.ajax({
			type 	: 'POST',
			url  	: base_url+'get_third_party_member',
			data 	: {id:id},
			success : function(data){
				data = JSON.parse(data);
    ...
    $(document).on('click','.reset-password',function(){
        ...
        $.ajax({
            type 	: 'POST',
            url  	: base_url+'third_party_member_reset_password',
            data 	: {id:id},
            success : function(data){
                data = JSON.parse(data);
	...
	$('#form_validation3').on('submit',function(e){
		...
		$.ajax({
			type 	: "POST",
			url  	: base_url+"update_third_party_member",
	...
	$(document).on('click','.delete-member',function(){
		...
		UIkit.modal.confirm('Are you sure to want delete this?', function(){ 
			$.ajax({
				type 	: 'POST',
				url  	: base_url+'delete_third_party_member',
				data 	: {id:id},
				success : function(data){
					data = JSON.parse(data);
    ...
    dt_tableExport: function() {
        ...
        if($dt_tableExport.length) {
            table_export = $dt_tableExport.DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": base_url+'get_third_party_members',
