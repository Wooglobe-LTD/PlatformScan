...
altair_mailbox = {
    ...
    list_switch: function() {
                ...
                begin: function() {
                    $mailbox2
                        ...
                        .velocity("transition.expandOut", {
                            ...
                            complete: function() {
                                $mailbox2.find('.md-card-list-header-combined').velocity("transition.expandIn", {
                                    duration: anim_duration,
                                    easing: easing_swiftOut
                                })
                            }
        ...
        $split_view_btn.on('click', function(e) {
            ...
            $split_view_btn.velocity("transition.expandOut", {
                ...
                easing: easing_swiftOut,
                begin: function() {
                    var $list = $mailbox2.find('.md-card-list'),
                    ...
                    setTimeout(function() {
                        $mailbox2.removeClass('md-card-list-combined');
                        $('.md-card-list-header')
        ...
        if($mailbox2.hasClass('md-card-list-combined')) {
    ...
    select_messages: function () {
        ...
        $mailbox2.on('ifChanged', '[data-md-icheck]', function() {
            $(this).is(':checked')
                ?
        ...
        $('#mailbox_select_all').on('ifChanged',function() {
            var $this = $(this);
            $mailbox2.find('[data-md-icheck]').each(function() {
                $this.is(':checked')
                    ?
                    $(this).iCheck('check')
                    :
    ...
    toggle_message: function() {
        ...
        $mailbox2.on('click', '.md-card-list ul > li', function(e) {
            ...
            if ( !$(e.target).closest('.md-card-list-item-menu').length && !$(e.target).closest('.md-card-list-item-select').length ) {
                ...
                if (!$this.hasClass('item-shown')) {
                    ...
                    // hide opened message
                    $mailbox2.find('.item-shown').velocity("reverse", {
