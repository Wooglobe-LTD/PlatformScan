...
$(function() {
	...
	$('#form_home_banner').on('submit',function(e){

		e.preventDefault();
		var form = $('#form_home_banner')[0];

		var data = new FormData(form);
		$.ajax({
			type 	: "POST",
			enctype: 'multipart/form-data',
			url  	: base_url+"update_home_banner",
			cache: false,
			contentType: false,
			processData: false,
			data    : data,
			success : function(data){
				...
				if(data.code == 204){
					...
					$('#err-msg').attr('data-message',data.message);
					$('#err-msg').click();
					$('#form_home_banner').parsley().reset();
					$.each(data.error,function(i,v){
						$('#'+i).parent().parent().find('.error').html(v);
					});
