...
$(document).on('click','.move-closewon',function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.code == 204) {
                            ...
                            var modal = UIkit.modal("#cancelform");
                            modal.hide();
                            window.location = base_url +"youtube_compilations";
                        } else {
                            $('#err-msg').attr('data-message', 'Something is going wrong!');
                            $('#err-msg').click();
                        }
        ...
        if (st > lastScrollTop) {
            ...
            if ((diff == $(this).outerHeight()) || (diff + 1 == $(this).outerHeight()) || (diff - 1 == $(this).outerHeight())) { // if scrolled to bottom
                if (stage_list[obj_index].max_offset_reached != 1) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url:  base_url + 'youtube_compilations',
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    Swal.fire({
                        ...
                        text: 'Lead Completed Successfully!',
                    });
                    setTimeout(function (){
    ...
    $('#video_compilation_claimed').click(function(e) {
        $.ajax({
            ...
            success: function (data) {
                ...
                setTimeout(function (){
                    window.location.href = base_url+'youtube_compilations';
                },5000)
    ...
    $(document).on('click','#leads-more',function (){
        var html = $('#more-section').html();
        v_index = (parseInt(v_index)+1);
        html = html.replaceAll('{VID}',v_index);
        $('#more-leads').append(html);
        $maskedInput = $('.masked_input');
        if($maskedInput.length) {
            $maskedInput.inputmask();
        }
        $('.select2_'+v_index).selectize();
    });
    $(document).on('click','.delete-lead-info',function (){
