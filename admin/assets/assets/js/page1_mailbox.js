/**
 * Created by Abdul Rehman Aziz on 5/10/2018.
 */
$(function() {
...
altair_mailbox = {
    ...
    list_switch: function() {
                ...
                begin: function() {
                    $mailbox1
                        ...
                        .velocity("transition.expandOut", {
                            ...
                            complete: function() {
                                $mailbox1.find('.md-card-list-header-combined').velocity("transition.expandIn", {
                                    duration: anim_duration,
                                    easing: easing_swiftOut
                                })
                            }
        ...
        $split_view_btn.on('click', function(e) {
            ...
            $split_view_btn.velocity("transition.expandOut", {
                ...
                easing: easing_swiftOut,
                begin: function() {
                    var $list = $mailbox1.find('.md-card-list'),
                    ...
                    setTimeout(function() {
                        $mailbox1.removeClass('md-card-list-combined');
                        $('.md-card-list-header')
        ...
        if($mailbox1.hasClass('md-card-list-combined')) {
    ...
    select_messages: function () {
        ...
        $mailbox1.on('ifChanged', '[data-md-icheck]', function() {
            $(this).is(':checked')
                ?
        ...
        $('#mailbox_select_all').on('ifChanged',function() {
            var $this = $(this);
            $mailbox1.find('[data-md-icheck]').each(function() {
                $this.is(':checked')
                    ?
                    $(this).iCheck('check')
                    :
    ...
    toggle_message: function() {
        ...
        $mailbox1.on('click', '.md-card-list ul > li', function(e) {
            ...
            if ( !$(e.target).closest('.md-card-list-item-menu').length && !$(e.target).closest('.md-card-list-item-select').length ) {
                ...
                if (!$this.hasClass('item-shown')) {
                    ...
                    // hide opened message
                    $mailbox1.find('.item-shown').velocity("reverse", {
