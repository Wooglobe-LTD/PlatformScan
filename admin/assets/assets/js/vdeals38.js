...
$(function() {
            ...
            success: function (data) {
                ...
                if (data.type == 'direct') {
                    ...
                    if (data.cmd != '') {
                        $.ajax({
                            type: "POST",
                            url: base_url + 'Video_Deals/ajax_create_zip_request',
                            data: {cmd: data.cmd},
                            success: function (data) {
                                console.log(data);
                                //data = JSON.parse(data);
    ...
    var clear_partners_specfiic_feeds = function () {
        ...
        $.ajax({
            type: 'GET',
            url: base_url + 'Video_Deals/clear_all_partner_feeds/'+video_id,
            async: false,
            success: function (data) {
                //data = JSON.parse(data);
            }
    ...
    $(document).on('click','#story_feed_save',function(){
        var categories = $('#general-categories-story').find('option:selected');
        var feeds = $('#story_feed_id').find('option:selected');
        if(categories.length == 0){
            alert('Please select at least one General Cateory');
            return false;
        }
        if(feeds.length == 0){
...
*/
    ...
    $(document).on('click','.distribute',function(e){
        ...
        if(url == 'publish-youtube'){
            ...
            $.ajax({
                url: base_url + 'Video_Deals/publishing_form_fields',
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.code == 204) {
                            ...
                            var modal = UIkit.modal("#cancelform");
                            modal.hide();
                            window.location = base_url +"video_deals";
                        } else {
                            $('#err-msg').attr('data-message', 'Something is going wrong!');
                            $('#err-msg').click();
                        }
