...
$(function() {
    ...
    $('#add').on('click',function(){
        ...
        var modal = UIkit.modal("#add_task_model");
        modal.show();
    ...
    $('#ma_form_validation2').on('submit',function(e){
        ...
        $.ajax({
            type 	: "POST",
            url  	: base_url+"mobile-app-add_task",
            processData: false,
            contentType: false,
            data    : formData,
    ...
    $(document).on('click','.ma-edit-task',function(){
        ...
        $.ajax({
            type 	: 'POST',
            url  	: base_url+'mobile-app-get_task',
            data 	: {id:id},
            success : function(data){
                data = JSON.parse(data);
                ...
                if(data.code == 204){
                    ...
                    var modal = UIkit.modal("#edit_task_model");
                    modal.show();
    ...
    $('#ma_form_validation3').on('submit',function(e){
        ...
        $.ajax({
            type 	: "POST",
            url  	: base_url+"mobile-app-update_task",
    ...
    $(document).on('click','.ma-delete-task',function(){

        var id = $(this).data('id');
        UIkit.modal.confirm('Are you sure to want delete this?', function(){
            $.ajax({
                type 	: 'POST',
                url  	: base_url+'mobile-app-delete_task',
                data 	: {id:id},
                success : function(data){
                    data = JSON.parse(data);
    ...
    dt_tableExport: function() {
        ...
        if($dt_tableExport.length) {
            table_export = $dt_tableExport.DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": base_url+'mobile-app-get_tasks',
