...
$(function() {
    ...
    parent = $('#parent_id_e').selectize();

    if(parent.length > 0){
        parent = parent[0].selectize;
    }

    parentAdd = $('#parent_id').selectize();

    if(parentAdd.length > 0){
        parentAdd = parentAdd[0].selectize;
    }
    ...
    $('#ma_form_validation2').on('submit',function(e){
        ...
        $.ajax({
            type 	: "POST",
            url  	: base_url+"mobile-app-add_category",
            data    : $('#ma_form_validation2').serialize(),
    ...
    $(document).on('click','.ma-edit-category',function(){
        ...
        $.ajax({
            type 	: 'POST',
            url  	: base_url+'mobile-app-get_category',
            data 	: {id:id},
            success : function(data){
                data = JSON.parse(data);
    ...
    $('#ma_form_validation3').on('submit',function(e){
        ...
        $.ajax({
            type 	: "POST",
            url  	: base_url+"mobile-app-update_category",
    ...
    $(document).on('click','.ma-delete-category',function(){

        var id = $(this).data('id');
        UIkit.modal.confirm('Are you sure to want delete this?', function(){
            $.ajax({
                type 	: 'POST',
                url  	: base_url+'mobile-app-delete_category',
                data 	: {id:id},
                success : function(data){
                    data = JSON.parse(data);
    ...
    dt_tableExport: function() {
        ...
        if($dt_tableExport.length) {
            table_export = $dt_tableExport.DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": base_url+'mobile-app-get_categories',
