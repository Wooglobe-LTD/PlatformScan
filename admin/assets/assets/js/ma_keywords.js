...
$(function() {
    ...
    $('#ma_form_validation2').on('submit',function(e){
        ...
        $.ajax({
            type 	: "POST",
            url  	: base_url+"mobile-app-add_keyword",
            processData: false,
            contentType: false,
            data    : formData,
    ...
    $(document).on('click','.ma-edit-keyword',function(){
        ...
        $.ajax({
            type 	: 'POST',
            url  	: base_url+'mobile-app-get_keyword',
            data 	: {id:id},
            success : function(data){
                data = JSON.parse(data);
    ...
    $('#ma_form_validation3').on('submit',function(e){
        ...
        $.ajax({
            type 	: "POST",
            url  	: base_url+"mobile-app-update_keyword",
    ...
    $(document).on('click','.ma-delete-keyword',function(){

        var id = $(this).data('id');
        UIkit.modal.confirm('Are you sure to want delete this?', function(){
            $.ajax({
                type 	: 'POST',
                url  	: base_url+'mobile-app-delete_keyword',
                data 	: {id:id},
                success : function(data){
                    data = JSON.parse(data);
    ...
    dt_tableExport: function() {
        ...
        if($dt_tableExport.length) {
            table_export = $dt_tableExport.DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": base_url+'mobile-app-get_keywords',
