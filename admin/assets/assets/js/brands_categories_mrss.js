...
$(function() {
    ...
    brand = $('#brand_e, #brand').selectize();

    if(brand.length > 0){
        brand = brand[0].selectize;
    }
    ...
    $('#form_validation2').on('submit',function(e){
		...
		$.ajax({
			type 	: "POST",
			url  	: base_url+"add_brands_category_mrss",
	...
	$(document).on('click','.edit-category',function(){
		...
		$.ajax({
			type 	: 'POST',
			url  	: base_url+'get_brands_category_mrss',
			data 	: {id:id},
			success : function(data){
				data = JSON.parse(data);
				if(data.code == 204){
					...
					$.each(data.data,function(i,v){
                        ...
                        // }
						if(i == 'partner' ){
                            $('#partner_e').data('selectize').setValue(v);
                            $("#partner_e").prop('required',true);
                            $("#partner_e").prop('readOnly',true);
                            $('#partner_e').closest('.uk-grid').show();
						}
                        if(i == 'brand'){
                            $('#brand_e').data('selectize').setValue(v);
                            $("#brand_e").prop('required',true);
                            $("#brand_e").prop('readOnly',true);
                            $('#brand_e').closest('.uk-grid').show();
						}
                        if(i == 'name' && v == '1'){
	...
	$('#form_validation3').on('submit',function(e){
		...
		$.ajax({
			type 	: "POST",
			url  	: base_url+"update_brands_category_mrss",
	...
	$(document).on('click','.delete-category',function(){
		...
		UIkit.modal.confirm('Are you sure to want delete this?', function(){
			$.ajax({
				type 	: 'POST',
				url  	: base_url+'delete_brands_category_mrss',
				data 	: {id:id},
				success : function(data){
					data = JSON.parse(data);
    ...
    $(document).on('click','.preview-feed',function(){
        ...
        $.ajax({
            type 	: 'POST',
            url  	: base_url+'get_brands_feed_data',
    ...
    dt_tableExport: function() {
        ...
        if($dt_tableExport.length) {
            table_export = $dt_tableExport.DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": base_url+'get_brands_categories_mrss',
