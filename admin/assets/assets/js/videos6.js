...
var statuss;
var video_type_id;
var youtube_publish_reson_yes;
var youtube_publish_reson_no;
var video_type = 1;
var media_id;
var mrss_cc;
$(function() {
    ...
    youtube_publish_reson_yes = $('#youtube_publish_reson_yes').selectize();
    if(youtube_publish_reson_yes.length > 0){
        youtube_publish_reson_yes = youtube_publish_reson_yes[0].selectize;
    }
    youtube_publish_reson_no = $('#youtube_publish_reson_no').selectize();
    if(youtube_publish_reson_no.length > 0){
        youtube_publish_reson_no = youtube_publish_reson_no[0].selectize;
    }
    ...
    reuters_catgory = $('#reuters_category_id').selectize({
        plugins: {
            'remove_button': {
                label: 'X'
            }
	...
	if(reuters_catgory.length > 0){
		reuters_catgory = reuters_catgory[0].selectize;
	}
...
/*	video_type_id = $('#video_type_id').selectize();
    ...
    $(document).on('ifChanged','[name="is_youtube_publish"]',function (){
        var val = $(this).val();
        $('.yt_publish').hide();
        if(val == "Yes"){
            $('#youtube_publish_reson_yes_div').show();
        }else{
            $('#youtube_publish_reson_no_div').show();
        }
    });
/*	video_type_id.on('change', function() {
  ...
  /*  if ($('input.delete_youtube_video').is(':checked')) {
   ...
   /* $(document).on('click','.check',function(e){
    ...
    $('#sign_appreance').on('ifUnchecked',function(e) {
        $('#release_description_div').hide();
        $('#release_description').val('');

    });
    $('#sign_appreance').on('ifChecked',function(e) {
        $('#release_description_div').show();
    });
    
    $('#Signer_Required').on('ifUnchecked',function(e) {
        $('#value_div').hide();

    });
    $('#Signer_Required').on('ifChecked',function(e) {
        $('#value_div').show();
    });

    $('#outro_video').on('ifUnchecked',function(e) {
        $('#outro_video_div').hide();

    });
    $('#outro_video').on('ifChecked',function(e) {
        $('#outro_video_div').show();
    });

    $('#video_credits').on('ifUnchecked',function(e) {
        $('#video_credits_div').hide();
    });
    $('#video_credits').on('ifChecked',function(e) {
        $('#video_credits_div').show();
    });
    $('#watermark').on('ifChecked',function(e) {
        $('#watermark_div').show();
    });
    $('#watermark').on('ifUnchecked',function(e) {
        $('#watermark_div').hide();
    });
    ...
    if(typeof exfedid !== 'undefined' && exfedid.length > 0){
