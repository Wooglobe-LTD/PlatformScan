...
$(function() {
	...
	roles = $('#roles_e').selectize();

	if(roles.length > 0){
		roles = roles[0].selectize;
	}

	staffs = $('#staffs_e').selectize();

	if(staffs.length > 0){
		staffs = staffs[0].selectize;
	}
	...
	roleAdd = $('#roles').selectize();

	if(roleAdd.length > 0){
		roleAdd = roleAdd[0].selectize;
	}

	staffAdd = $('#staffs').selectize();

	if(staffAdd.length > 0){
		staffAdd = staffAdd[0].selectize;
	}
    ...
    $(document).on('click','.group-permissions',function(){
        ...
        window.location = base_url+'group_permissions/'+id;
