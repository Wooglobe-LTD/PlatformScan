...
class APP_Controller extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        foreach($this->data['menu_list'] as $parent){
            if (get_sub_menu($parent->id) != NULL) {
                $this->data[$parent->menu_name] = get_sub_menu($parent->id);
            }
        }
    ...
    public function email($to,$to_name = '',$from,$from_name = '',$subject,$message,$cc = '',$bcc = '',$replyto = '',$replyto_name = '',$file_to_attach='',$unique_key='')
    {
        ...
        try {
            ...
            $mail->Username = $from == "PartnerSupport@WooGlobe.com"? 'PartnerSupport@WooGlobe.com': 'viral@wooglobe.com';    // SMTP username
            $mail->Password = $from == "PartnerSupport@WooGlobe.com"? 'Rizkhan183': 'bbospbhczbaffjxu';                       // SMTP password
    ...
    public function get_s3_client2()
    {
        $config = array(
            /*'key' => S3_KEY,
            'secret' => S3_SECRET,*/
    ...
    public function upload_file_s3_pri($source_path, $target_file_key, $file_extension, $delete_source = FALSE,
                                   $app_user_id = NULL)
    {
...
//
    ...
    public function get_file_s3_pri($target_file_key){
        $s3 = $this->get_s3_client2();
    ...
    public function get_file_s3($target_file_key){
        ...
        $request = $s3->createPresignedRequest($cmd, '+3 minutes');
        $presignedUrl = (string)$request->getUri();
        return $presignedUrl;

    }
    public function download_file_s3($target_file_key, $local_file_path) {
        $s3 = $this->get_s3_client();
        $s3->getObject([
            'Bucket' => S3_BUCKET,
            'Key' => $target_file_key,
    ...
    public function get_file_info($target_file_key){
        $s3 = $this->get_s3_client();
        $info = $s3->doesObjectExist(S3_BUCKET, $target_file_key);
    ...
    public function s3_change_permission_file($target_file_key,$permission)
    {
        $s3 = $this->get_s3_client2();

       $res =  $s3->putObjectAcl(array(
            'Bucket' => 'wgabucket',
            'Key' => $target_file_key,
            'ACL' => $permission
        ));
    ...
    public function s3_change_permission_file_2($target_file_key,$permission)
    {
        $s3 = $this->get_s3_client();
    ...
    public function upload_file_s3($source_path, $target_file_key, $file_extension, $delete_source = FALSE,
                                   $app_user_id = NULL)
    {
        if(file_exists($source_path)) {
...
//        }
    ...
    public function upload_file_s3_new($permission = 'public-read',$source_path, $target_file_key, $file_extension, $delete_source = FALSE,
                                   $tag = "Video")
    {
        if(file_exists($source_path)) {
...
//        }
    ...
    public function get_dropbox_client(){
        // $app = new DropboxApp($GLOBALS["DROPBOX_KEY"], $GLOBALS["DROPBOX_SECRET"]);
        $app = new DropboxApp("acuytfy91crp1xb", "9wmi20ycocatamp");

        $dropbox = new Dropbox($app);

        if(!$this->sess->userdata('DROPBOX_ACCESS_TOKEN')){
            // print_r($GLOBALS["DROPBOX_ACCESS_TOKEN"]);
            
            $authHelper = $dropbox->getAuthHelper();
            $redirect_uri = base_url()."dropbox_access_token"; 
            // return $redirect_uri;
            $authUrl = $authHelper->getAuthUrl($redirect_uri, [], NULL, "offline");
            
            // redirect($authUrl);
            return $authUrl;
        }
        else{
            $dropbox->setAccessToken($this->sess->userdata('DROPBOX_ACCESS_TOKEN'));
            try{
                $result = $dropbox->getCurrentAccount();
            }
    ...
    public function add_entry_to_sheet($url, $timestamp_from, $timestamp_to, $wgid, $predefined, $dropbox_url){
        $this->sheet->addRowToSpreadsheet(array($url, $timestamp_from . " - " . $timestamp_to, $wgid, $predefined, $dropbox_url), 357124305);
    }
    public function update_dropbox_upload_status(){
        $this->load->library('sheet');

        $result = $this->db
            ->where('dropbox_status !=', NULL)
            ->where('dropbox_status !=', 'success')
            ->where('dropbox_status !=', 'failed')
            ->get("raw_video");
        ...
        foreach($result->result_array() as $row){
            $job_status = $client->checkJobStatus($row['dropbox_status']);
            $s = NULL;
            if ($job_status instanceof \Kunnu\Dropbox\Models\FileMetadata) {
                $s = "success";
                $shareable_link = "";

                $wgid = $this->db
                ->where('id =', $row['lead_id'])
                ->get("video_leads")->result_array()[0];
                
                if ($wgid["lead_duration"] < 20){ 

                    $wgid = $wgid['unique_key'];
                    $folder_path = $job_status->getPathLower();
                    try{
                        $response = $client->postToApi("/sharing/list_shared_links", ["path" => $folder_path])->getDecodedBody();
                        if($response["links"] != NULL){
                            $shareable_link = $response["links"][0]["url"];
                        }
                        if($shareable_link == "" || $shareable_link == NULL){
                            $response =  $client->postToAPI("/sharing/create_shared_link_with_settings", ["path" => $folder_path, "settings" => ['requested_visibility' => 'public']])->getDecodedBody();
                            $shareable_link = $response["url"];
                        }
                    }
                    catch (\Kunnu\Dropbox\Exceptions\DropboxClientException $e){
                        if($e->getCode() == 409){
                            // Share Link Already Generated || Path Not Found
                        }
                    }
                    $result = $this->db->where("id =", $row["video_id"])->get("videos");
                    if($result){
                        $youtube_id = $result->result_array()[0]["youtube_id"];
                        if($row["is_enter_sheet"] == 1){
                            $this->sheet->addRowToSpreadsheet(array('https://www.youtube.com/watch?v='.$youtube_id, "Complete Video", $wgid, "WooGlobe M DRM", $shareable_link), 357124305);
                        }
                    }
                }
            ...
            if($s != NULL){
                $this->db->set('dropbox_status', $s)
                    ->where('s3_url', $row["s3_url"])
                    ->update('raw_video');
            }
    ...
    public function upload_file_dropbox_new($source_path, $target_path, $lead_id, $is_edited, $is_enter_sheet = 0){
        $dropbox = $this->get_dropbox_client();
        try{
            $status = $dropbox->saveUrl($target_path, $source_path);
        } catch(Exception $e){
            return false;
        }
        ...
        if($status != NULL){
            $data = array(
                ...
                'is_enter_sheet' => $is_enter_sheet
            );
            if($is_edited){
                $this->db
                // ->set('dropbox_status', $status)
                    ->where('lead_id', $lead_id)
                    ->update('raw_video', $data);
            } else{
                $this->db
                // ->set('dropbox_status', $status)
                    ->where('s3_url', $source_path)
                    ->update('raw_video', $data);
            }
        }
    ...
    public function search_dropbox_file($unique_key, $path = '/') {
        $dropbox = $this->get_dropbox_client();
        ...
        $response = $dropbox->search($path, $unique_key);
        $matches = $response->matches; 
        foreach ($matches as $match) {
            $metadata = $match['metadata'];
            $path = str_replace($metadata['name'], '', $metadata['path_display']);
            $excluded = false;
            foreach($exluded_folders as $ex) {
                if (strpos($path, $ex) !== false) {
                    $excluded = true;
                }
            }
            if ($metadata['.tag'] === 'file' && !$excluded) {
                $files[] = $path;
            }
        }
    ...
    public function get_job_status($job_id) {
        $dropbox = $this->get_dropbox_client();
        $response = $dropbox->checkJobStatus($job_id);
        
        return $response;
    }

    public function upload_zip_file2($source_path, $target_file_key, $delete_source = FALSE, $app_user_id = NULL)
    {

        $s3 = $this->get_s3_client();

        $upload = $s3->upload(S3_BUCKET, $target_file_key, fopen($source_path, 'rb'), 'public-read');
        /*
          $uploader = new MultipartUploader($s3, $source_path, array(
          'bucket' => S3_BUCKET,
          'key' => $target_file_key,
          ));
    ...
    public function upload_zip_file2_new($permission = 'public-read',$source_path, $target_file_key, $delete_source = FALSE, $app_user_id = NULL)
    {

        $s3 = $this->get_s3_client();

        $upload = $s3->upload(S3_BUCKET, $target_file_key, fopen($source_path, 'rb'), $permission);
        /*
          $uploader = new MultipartUploader($s3, $source_path, array(
          'bucket' => S3_BUCKET,
          'key' => $target_file_key,
    ...
    public function upload_zip_file($source_path, $target_file_key, $delete_source = FALSE, $app_user_id = NULL)
    {
    ...
    public function upload_zip_file_new($permission = 'public-read',$source_path, $target_file_key, $delete_source = FALSE, $app_user_id = NULL)
    {
    ...
    public function awsMediaConvert($s3_source,$source,$uId){
        ...
        if(isset($file['filesize']) && $file['filesize'] > 0){
            ...
            if(isset($file['video']['rotate']) && $file['video']['rotate'] == 90 || $file['video']['rotate'] == 270)
               {
        ...
        $videoTemplate['Settings']['Inputs'][0]['ImageInserter']['InsertableImages'][0]['ImageInserterInput'] = 's3://wooglobe/video_encoding/current watermark.png';
    ...
    public function awsMediaConvertSocial($s3_source,$source,$uId,$inputs){
        ...
        $per20 = ((20/100)*$resolution_x);
        $per30 = ((30/100)*$resolution_y);
        $per3 = ((3/100)*$resolution_x);
        $perH5 = ((5/100)*$resolution_y);
        $perW3 = ((3/100)*$resolution_x);
        $factor = @(722/$per20);
        $factorIcon = @(1280/$per20);
        $factorCredits = @(300/$per30);
        $waterMarkW = (int)(722/$factor);
        $waterMarkH = (int)(170/$factor);

        $iconW = (int)(1280/$factorIcon);
        $iconH = (int)(720/$factorIcon);

        $creditsW = (int)(36/$factorCredits);
        $creditsH = (int)(300/$factorCredits);

        $layer = 2;
        $videoTemplate = json_decode(file_get_contents('./video_tem/watermark.json'),true);
        $outroTemplate = json_decode(file_get_contents('./video_tem/outro.json'),true);
        ...
        $videoTemplate['Settings']['OutputGroups'][0]['Outputs'][0]['NameModifier'] = "_$uId";
        //unset($videoTemplate['Settings']['OutputGroups'][0]['Outputs'][0]['NameModifier']);
        $imageArray = $videoTemplate['Settings']['Inputs'][0]['ImageInserter']['InsertableImages'][0];
        $outroFile = $outroTemplate['Settings']['Inputs'][1];

        $videoTemplate['Settings']['Inputs'][0]['ImageInserter']['InsertableImages'] = [];
        if (isset($inputs['watermark']) && $inputs['watermark'] == 1) {

            $waterMarkArray = $imageArray;


            $waterMarkArray['Width'] = $waterMarkW;
            $waterMarkArray['Height'] = $waterMarkH;
            $waterMarkArray['ImageX'] = (int)(($resolution_x - $waterMarkW) - $per3);
            $waterMarkArray['ImageY'] = (int)$perH5;
            $waterMarkArray['Duration'] = $seconds;
            $waterMarkArray['Layer'] = $layer;
            //$waterMarkArray['Opacity'] = 70;
            $waterMarkArray['ImageInserterInput'] = 's3://wooglobe/video_encoding/current watermark.png';
            $videoTemplate['Settings']['Inputs'][0]['ImageInserter']['InsertableImages'][] = $waterMarkArray;
            $layer = ($layer+1);
        }

        if ((isset($inputs['video_credits']) && $inputs['video_credits'] == 1) && (isset($inputs['video_credits_value']) && !empty($inputs['video_credits_value']))) {
            $creditsImage = "./video_tem/$inputs[video_credits_value].png";
            $img = imagecreatefrompng($creditsImage);
            imagesavealpha($img, true);
            imagealphablending($img, false);
            $white = imagecolorallocate($img, 255, 255, 255);
            $font = "./video_tem/arial.ttf";
            imagettftext($img, 24, 90, 28, 265, $white, $font, $inputs['credit_user_name']);
            imagepng($img, "./video_tem/credits.png");
            imagedestroy($img);
            $this->upload_file_s3_new('public-read',"./video_tem/credits.png",'video_encoding/credits.png','png',true);

            $creditskArray = $imageArray;

            $creditskArray['Width'] = $creditsW;
            $creditskArray['Height'] = $creditsH;
            $creditskArray['ImageX'] = (int)(($resolution_x - $creditsW) - 10);
            $creditskArray['ImageY'] = (int)(($resolution_y/2)-($creditsH/2));
            $creditskArray['Duration'] = $seconds;
            $creditskArray['Layer'] = $layer;
            $waterMarkArray['Opacity'] = 70;
            $creditskArray['ImageInserterInput'] = 's3://wooglobe/video_encoding/credits.png';
            $videoTemplate['Settings']['Inputs'][0]['ImageInserter']['InsertableImages'][] = $creditskArray;
            $layer = ($layer+1);
        }
       /* if (isset($inputs['social_icon']) && $inputs['social_icon'] == 1) {
            $iconArray = $imageArray;


            $iconArray['Width'] = $iconW;
            $iconArray['Height'] = $iconH;
            $iconArray['ImageX'] = (int)(($resolution_x/2) - ($iconW/2));
            $iconArray['ImageY'] = (int)(($resolution_y - $iconH)-10);
            $iconArray['Duration'] = $seconds;
            $iconArray['Layer'] = $layer;
            $waterMarkArray['Opacity'] = 70;
            $iconArray['ImageInserterInput'] = 's3://wooglobe/video_encoding/bottom_social.png';
            $videoTemplate['Settings']['Inputs'][0]['ImageInserter']['InsertableImages'][] = $iconArray;
            $layer = ($layer+1);
        }*/
        if (isset($inputs['sound_mute']) && $inputs['sound_mute'] == 1) {

            unset($videoTemplate['Settings']['OutputGroups'][0]['Outputs'][0]['AudioDescriptions']);
        }


        if ((isset($inputs['outro_video']) && $inputs['outro_video'] == 1) && (isset($inputs['outro_video_value']) && !empty($inputs['outro_video_value']))) {
            $outroResult = $this->db->query("SELECT * FROM outro_videos WHERE width = $resolution_x AND height = $resolution_y AND type = '$inputs[outro_video_value]'");

            $data = [];
            if($outroResult->num_rows() == 0){

                $outroDbData['width'] = $resolution_x;
                $outroDbData['height'] = $resolution_y;
                $outroDbData['type'] = $inputs['outro_video_value'];
                $outroDbData['s3_path'] = $inputs['outro_video_value']."_".$resolution_x."_".$resolution_y.".mp4";

                $shellCommand = "ffmpeg -i /var/www/html/admin/video_tem/$inputs[outro_video_value].mp4 -vf scale=$resolution_x:$resolution_y,setsar=1:1 /var/www/html/admin/video_tem/$inputs[outro_video_value]_".$resolution_x."_".$resolution_y.".mp4";

                $data = $outroDbData;
                exec($shellCommand,$out,$ret_var);
                /*echo '<pre>';
                print_r($ret_var);
                echo '</pre>';
                exit;*/
                if ($ret_var == 0)
                {
                    $this->upload_file_s3_new('public-read',"./video_tem/$inputs[outro_video_value]_".$resolution_x."_".$resolution_y.".mp4","video_encoding/$inputs[outro_video_value]/$inputs[outro_video_value]_".$resolution_x."_".$resolution_y.".mp4",'mp4',true);
                    $this->db->insert('outro_videos',$outroDbData);
                    $outro = "s3://wooglobe/video_encoding/$inputs[outro_video_value]/$outroDbData[s3_path]";
                    $outroFile['FileInput'] = $outro;
                    $videoTemplate['Settings']['Inputs'][] = $outroFile;
                }else{
                    $outro = "s3://wooglobe/video_encoding/$inputs[outro_video_value].mp4";
                    $outroFile['FileInput'] = $outro;
                    $videoTemplate['Settings']['Inputs'][] = $outroFile;
                }
                /*echo "<pre>".$result."</pre>";
                ...
                $data = (array) $outroResult->row();
                $outro = "s3://wooglobe/video_encoding/$data[type]/$data[s3_path]";
                $outroFile['FileInput'] = $outro;
                $videoTemplate['Settings']['Inputs'][] = $outroFile;
        ...
        //$destination = explode('.',$destination);
        //$destination =$destination[0];
        $destination = str_replace('raw_videos','social_video',$destination);
        $destination = $videoTemplate['Settings']['OutputGroups'][0]['OutputGroupSettings']['FileGroupSettings']['Destination'].$destination;
        $videoTemplate['Settings']['OutputGroups'][0]['OutputGroupSettings']['FileGroupSettings']['Destination'] = $destination;
        /*echo '<pre>';
        print_r($videoTemplate['Settings']['OutputGroups'][0]['OutputGroupSettings']['FileGroupSettings']['Destination']);
