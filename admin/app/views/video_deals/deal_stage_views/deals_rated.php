...
<?php foreach($deals_rated->result() as $rated){
    ...
    <div class="sub-scrum">
        <div class="scrum_task critical <?php if($rated->reminder_sent > 0) {echo "reminder_".$rated->reminder_sent;}?>" <?php if($rated->simple_video == 1){ echo 'style="background: #C8C8C8;"'; } ?>>
            ...
            <?php if(!empty($rated->report_issue_type)){ ?>
                <div style="width: 100%;text-align: right"><p class="uk-button uk-button-danger" data-uk-tooltip title="<b>Types : <?php $types = explode(',',$rated->report_issue_type); foreach ($types as $type){ echo $type.'<br>';} ?></b><br><?php echo $rated->report_issue_desc;?>">Issue</p></div>
            <?php } ?>
