...
class CommentsController extends APP_Controller {

    public function __construct() {
        parent::__construct();
        $this->data['active'] = 'Mobile App Comments';
        $css = array(
        ...
        $this->load->model('MobileAppComment','user');
    ...
    public function index()
    {
        ...
        $this->data['content'] = $this->load->view('mobile_app/comments/listing',$this->data,true);
    ...
    public function comments_listing(){
        ...
        $result = $this->user->getAllComments(3,'id,full_name,gender,email,mobile,(case when (status = 1) THEN "Active" ELSE "Inactive" END) as status',$search,$start,$limit,$orderby,$params['columns'],3);
        $resultCount = $this->user->getAllComments(3);
        $response = array();
        $data = array();
        ...
        foreach($result->result() as $row){
            ...
            if($this->data['assess']['can_edit']) {
                $links .= '<a title="Edit Comment" href="javascript:void(0);" class="edit-comment" data-id="' . $row->id . '"><i class="material-icons">&#xE254;</i></a> ';
            }
            if($this->data['assess']['can_delete']) {
                $links .= '| <a title="Delete Comment" href="javascript:void(0);" class="delete-comment" data-id="' . $row->id . '"><i class="material-icons">&#xE92B;</i></a>';
            }
    ...
    public function get_comment(){
        ...
        $result = $this->user->getCommentById($id,'id,comment');
        if(!$result){
    ...
    public function update(){
        ...
        $response['url'] = '';
        $id = $this->security->xss_clean($this->input->post('id'));
        $result = $this->user->getCommentById($id);
        if(!$result){
        ...
        $this->validation->set_rules('comment','comment','required');
        ...
        if($this->validation->run() === false){
            ...
            $dbData['updated_by'] = $this->sess->userdata('adminId');
            $this->db->where('id',$id);
            $this->db->update('videos_comments',$dbData);
        }
