...
class PageController extends APP_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('MobileAPPPage','page');
    ...
    public function index()
    {
        ...
        $this->data['title'] = 'Mobile Pages Management';
        $this->data['content'] = $this->load->view('mobile_app/pages/listing',$this->data,true);
    ...
    public function pages_listing(){
        ...
        $result = $this->page->getAllPages('id,title,body,url,created_at,updated_at,(case when (status = 1) THEN "Active" ELSE "Inactive" END) as status', $search, $start, $limit, $orderby, $params['columns'], 3);
        ...
        foreach($result->result() as $row){
            ...
            if($this->data['assess']['can_edit']) {
                $links .= '<a title="Edit Page" href="' . base_url('mobile-edit_page/' . $row->id) . '" class="edit-page" data-id="' . $row->id . '"><i class="material-icons">&#xE254;</i></a> ';
            }
            if($this->data['assess']['can_edit']) {
    ...
    public function page_add()
    {
        ...
        $this->data['content'] = $this->load->view('mobile_app/pages/add',$this->data,true);
        $this->load->view('common_files/template',$this->data);
    }
    ...
    public function add_page(){
        ...
        $this->validation->set_rules('title','Page Title','trim|required|alpha_numeric_spaces');
        ...
        if($this->validation->run() === false){
            ...
            $fields = array('title','body','stauts');
            $errors = array();
    ...
    public function edit_page($id)
    {
        ...
        $this->data['content'] = $this->load->view('mobile_app/pages/edit',$this->data,true);
        $this->load->view('common_files/template',$this->data);
    }
    ...
    public function update_page(){
        ...
        $this->validation->set_rules('title','body','trim|required|alpha_numeric_spaces');
        //$this->validation->set_rules('description','Page Description','trim|required');
        ...
        if($this->validation->run() === false){
            ...
            $fields = array('title','status_u','body');
            $errors = array();
            foreach($fields as $field){
                $errors[$field] = form_error($field);
            }
            ...
            $response['url'] = $this->data['url'].'mobile-pages';
            $dbData = $this->security->xss_clean($this->input->post());
             $this->load->helper('text');
            $this->load->helper('url');
            $slug = url_title(convert_accented_characters($dbData['title']), 'dash', true);
             $dbData['url'] = $slug;
             $dbData["status"]=$dbData["status_u"];
             unset($dbData["status_u"]);

            $dbData['updated_at'] = date('Y-m-d H:i:s');
            $dbData['updated_by'] = $this->sess->userdata('adminId');
            $this->db->where('id',$id);
            $this->db->update('ma_pages',$dbData);
        }
