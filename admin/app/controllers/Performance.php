...
class Performance extends APP_Controller {
	...
	public function index()
	{
       ...
       /* if($role_id == 11) {*/
		...
		$this->data['content'] = $this->load->view('reports/performance',$this->data,true);
	...
	public function reports_listing(){
        ...
        if(isset($params['search2']) && $params['search2'] == 2) {
            ...
            if (isset($params['type']) && is_array($params['type'])) {
                ...
                foreach ($params['type'] as $staff) {
                    $staff_name = '';
                    $foreachWhere = '';
                    $foreachWhere = $where;
                    $staff_id = $staff;
                    if($staff == '-1'){
                        $foreachWhere .= " AND vl.staff_id IS NULL ";
                    }else{
                        $foreachWhere .= " AND vl.staff_id = $staff ";
                    }

                    $check_query = 'select name from admin where id=' . $staff;
                    $check_res = $this->db->query($check_query);
                    if ($check_res->num_rows() > 0) {
                        $staff_name = $check_res->row()->name;
                    }else{
                    ...
                    $resultCount = $this->report->getReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);
                    $signedCount = $this->report->getsignedReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);
                    $rateCount = $this->report->getNotRateReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);
                    $actionCount = $this->report->getTakeActionReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);
                    $signedRejectCount = $this->report->getsignedAndrejectReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);
                    $acquiredRejectCount = $this->report->getAcquiredReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);

                    $rejectedCount = $this->report->getrejectedReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);
                    $canceledCount = $this->report->getCancelReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);
                    $poorCount = $this->report->getPoorReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);
                    $notCount = $this->report->getNotInterestReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $foreachWhere);
    ...
    public function reports_details(){
        ...
        if(isset($params['staff_id'])){
            $check_query = 'select * from admin where id=' . $params['staff_id'];
            $check_res = $this->db->query($check_query);
            $check_role_id = $check_res->row();
            $role_id = $check_role_id->admin_role_id;
            $staff_name = $check_role_id->name;
        }else{
            $role_id = 0;
            $staff_name = 'Wooglobe';
        }
        ...
        if(isset($params['search2']) && $params['search2'] == 2) {
            ...
            $response = array();
            $data = array();
            $data[] = ['WG ID','Deal Name','Acquired Date','Stage','Created Time','Rating'];
        ...
        $now = date('m_d_Y');
        $filename = './report/'.$staff_name.'_'.$now.'performance_report.csv';
        $url = base_url('report/'.$staff_name.'_'.$now.'performance_report.csv');
    ...
    public function reports_details2(){
        $params = $this->security->xss_clean($this->input->get());
        $adminid = $this->sess->userdata('adminId');
        ...
        //if(isset($params['search2']) && $params['search2'] == 2) {
            ...
            $response = array();
            $data = array();
            //$data[] = ['WG ID','Deal Name','Acquired Date','Stage','Created Time'];
    ...
    public function pie_report(){
        ...
        if (isset($params['type']) && !empty($params['type'])) {
            if ($params['type'] == '-1') {
                $staff_name = 'WooGlobe';
                $pieWhere .= " AND vl.staff_id IS NULL";
            } else {
                $pieWhere .= " AND vl.staff_id in (".implode(',',$params['type']).")";
        ...
        $chartLabel = "All Leads";
        if (isset($params['graph']) && !empty($params['graph']) && $params['graph'] != 'lead') {
            if($params['graph'] == 'not_rate'){
                $chartLabel = "Not Rated Leads";
                $where .= " AND vl.deleted = 0 and vl.status = 1 ";
            }else if($params['graph'] == 'action'){
                $chartLabel = "Action Required Deals";
                $where .= " vl.deleted = 0 and vl.status = 10 ";
            }else if($params['graph'] == 'signed'){
                $chartLabel = "Signed (Awaiting Approval) Deals";
                $where .= " AND vl.status in (3)
		    AND vl.load_view != 5 ";
            }else if($params['graph'] == 'signed_reject'){
                $chartLabel = "Signed (Rejected, Awaiting Resolution) Deals";
                $where .= " AND vl.status in (3)
		    AND vl.load_view = 5 ";
            }else if($params['graph'] == 'acquired'){
                $chartLabel = "Acquired Deals";
                $where .= " AND vl.deleted = 0 AND vl.status in (6,7,8) ";
            }else if($params['graph'] == 'cancel'){
                $chartLabel = "Canceled Deals";
                $where .= " AND vl.deleted = 1 ";
            }else if($params['graph'] == 'poor'){
                $chartLabel = "Poor Deals";
                $where .= " AND vl.deleted = 0 and vl.status = 5 ";
            }else if($params['graph'] == 'not'){
                $chartLabel = "Not Interested Deals";
                $where .= " AND vl.deleted = 0 and vl.status = 11 ";
            }
        }
        ...
        //if($role_id == '11'){
        $pieChart = $this->report->getPieMultiReports('*', '', 0, 0, 'vl.id DESC',$params['columns'], $where.$pieWhere);
        ...
        if (isset($params['type']) && !empty($params['type'])) {
            if ($params['type'] == '-1') {
                ...
                $gWhere .= " AND vl.staff_id in (".implode(',',$params['type']).")";
            }
        }
        $weeklyChart = array();
        $monthlyChart = array();
        if (isset($params['type']) && is_array($params['type'])) {
            ...
            foreach ($params['type'] as $j=> $staff) {
                $weeklyData = array();
                $monthlyData = array();
                $weekly = array();
                $staff_name = '';
                ...
                $weeklyData = array('type'=>'line','showInLegend'=>true,'name'=>"series $j",'legendText'=>$staff_name);
                $now = new DateTime("7 days ago", new DateTimeZone('America/New_York'));
                $interval = new DateInterval('P1D'); // 1 Day interval
                $period = new DatePeriod($now, $interval, 7); // 7 Days
                $i = 0;
                foreach ($period as $day) {
                    ...
                    $key = $day->format('Y-m-d');
                    $where_date = $foreachWhere." AND DATE(vl.created_at) = '$key'";
                    $where_update_date = $foreachWhere." AND DATE(vl.updated_at) = '$key'";
                    $where_lead = $foreachWhere." AND vl.status >= 1 AND DATE(vl.created_at) = '$key'";
                    $day_key = date('l', strtotime($key));
                    $day_date = date('Y-m-d', strtotime($key));
                    if (isset($params['graph']) && !empty($params['graph']) && $params['graph']) {
                        if($params['graph'] == 'not_rate'){
                            $resultCount = $this->report->getNotRateReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $where_date);
                        }else if($params['graph'] == 'action'){
                            $resultCount = $this->report->getTakeActionReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $where_date);
                        }else if($params['graph'] == 'signed'){
                            $resultCount = $this->report->getsignedReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $where_date);
                        }else if($params['graph'] == 'signed_reject'){
                            $resultCount = $this->report->getsignedAndrejectReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $where_date);
                        }else if($params['graph'] == 'acquired'){
                            $resultCount = $this->report->getAcquiredReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $where_date);
                        }else if($params['graph'] == 'cancel'){
                            $resultCount = $this->report->getCancelReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $where_date);
                        }else if($params['graph'] == 'poor'){
                            $resultCount = $this->report->getPoorReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $where_date);
                        }else if($params['graph'] == 'not'){
                            $resultCount = $this->report->getNotInterestReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $where_date);
                        }else if($params['graph'] == 'lead'){
                            $resultCount = $this->report->getleadReports('*', '', 0, 0, 'vl.id DESC', '', $where_date);
                        }
                    }

                    $weekly[$i]['y'] = $resultCount->num_rows();
                    $weekly[$i]['label'] = $day_key . '-' . $day_date;

                    $i++;

                }
                $weeklyData['dataPoints'] = $weekly;
                $weeklyChart[] = $weeklyData;
                $monthData = array('type'=>'line','showInLegend'=>true,'name'=>"series $j",'legendText'=>$staff_name);
                $month_now = new DateTime("30 days ago", new DateTimeZone('America/New_York'));
                $month_interval = new DateInterval('P1D'); // 1 Day interval
                $month_period = new DatePeriod($month_now, $month_interval, 30); // 7 Days
                $i = 0;
                ...
                for($start_month; $start_month <= $end_month; $start_month++){
                    ...
                    //$month_key = $month_day->format('Y-m-d');
                    $month_where_date = $foreachWhere." AND DATE(vl.created_at) BETWEEN '$first_date' AND '$last_date'";
                    $month_where_update_date = $foreachWhere." AND DATE(vl.updated_at) BETWEEN '$first_date' AND '$last_date'";
                    $month_where_lead = $foreachWhere." AND vl.status >= 1 AND DATE(vl.created_at) BETWEEN '$first_date' AND '$last_date'";
                    $month_day_key = $m;

                    if (isset($params['graph']) && !empty($params['graph']) && $params['graph']) {
                        if($params['graph'] == 'not_rate'){
                            $resultCount = $this->report->getNotRateReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $month_where_date);
                        }else if($params['graph'] == 'action'){
                            $resultCount = $this->report->getTakeActionReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $month_where_date);
                        }else if($params['graph'] == 'signed'){
                            $resultCount = $this->report->getsignedReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $month_where_date);
                        }else if($params['graph'] == 'signed_reject'){
                            $resultCount = $this->report->getsignedAndrejectReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $month_where_date);
                        }else if($params['graph'] == 'acquired'){
                            $resultCount = $this->report->getAcquiredReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $month_where_date);
                        }else if($params['graph'] == 'cancel'){
                            $resultCount = $this->report->getCancelReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $month_where_date);
                        }else if($params['graph'] == 'poor'){
                            $resultCount = $this->report->getPoorReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $month_where_date);
                        }else if($params['graph'] == 'not'){
                            $resultCount = $this->report->getNotInterestReports('*', '', 0, 0, 'vl.id DESC', $params['columns'], $month_where_date);
                        }else if($params['graph'] == 'lead'){
                            $resultCount = $this->report->getleadReports('*', '', 0, 0, 'vl.id DESC', '', $month_where_date);
                        }
                    }
                    $month[$i]['y'] = $resultCount->num_rows();
                    $month[$i]['label'] = $month_day_key;
                    $i++;
                }
                $monthData['dataPoints'] = $month;
        ...
        $response['label'] = $chartLabel;
        $response['week'] = $weeklyChart;
        $response['month'] = $monthlyChart;
