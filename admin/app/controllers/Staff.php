...
class Staff extends APP_Controller {
	...
	public function members_listing(){
		...
		$result = $this->staff->getAllMembers('ar.title,a.id,a.username,a.name,a.email,a.link,a.simple_link,case when (a.status = 1) THEN "Active" ELSE "Inactive" END as status,a.admin_role_id',$search,$start,$limit,$orderby,$params['columns']);
		...
		foreach($result->result() as $row){
            ...
            $simple_share_link='';
            if(!empty($row->simple_link)){
                $simple_share_link= root_url().'submit-video-to-win/'.$row->simple_link;
            }
            $userRoles = getRolesIds($row->admin_role_id);
            $roles = '';
            foreach ($userRoles->result() as $ro){
                $roles .= $ro->title.'<br>';
            }
			$r[] = $roles;
			$r[] = $row->name;
	...
	public function add_member(){
        ...
        $this->validation->set_message('simple_link','This field is required.');
		$this->validation->set_message('alpha_numeric_spaces','Only alphabet and number are allowed.');

		if($this->validation->run() === false){
			
			$fields = array('admin_role_id','name','username','email','status','link','simple_link');
			$errors = array();
            ...
            $dbData['admin_role_id'] = implode(',',$dbData['admin_role_id']);
	...
	public function get_member(){
		...
		$result = $this->staff->getMemberById($id,'a.id,a.admin_role_id,a.status,a.name,a.username,a.email,a.link,a.simple_link');
        $result->admin_role_id = explode(',',$result->admin_role_id);
		if(!$result){
	...
	public function update_member(){
        ...
        $this->validation->set_rules('simple_link','Link','trim|required');
        $this->validation->set_message('required','This field is required.');
        $this->validation->set_message('alpha_numeric_spaces','Only alphabet and number are allowed.');

        if($this->validation->run() === false){

            $fields = array('admin_role_id','name','username','email','status','simple_link');
